"""TC-036: the same real-client, real-container proof TC-020 established for pdf/net
(``tests/e2e/test_container_session.py``) and TC-030 established for slides/python
(``tests/e2e/test_slides_python_container_session.py``), mirrored against the cells/rust
pilot's own published port. ``tests/mcp/test_server_wiring.py`` already proves the same
contract in-process (``TestClient``, no socket); this file is the one place that in-process
proof is checked against reality for cells/rust specifically, and the one place a broken
published port for THIS pilot is guaranteed to be noticed.
...
``serving-cells-rust`` (8086), deliberately never read back out of that file: a harness
...
CRITICAL, learned the hard way on TC-030: with an empty manifest store, every deployment
answers tool calls IDENTICALLY (isError: false, honest empty/NotAvailable results) regardless
of family/platform - so a test asserting only generic protocol behavior can PASS even when
pointed at the WRONG container, if another pilot's container happens to be running at the same
time (which it will be, since this file's check command runs alongside the other E2E files).
``test_report_index_freshness_identifies_the_real_cells_rust_deployment`` below exists
specifically to close that gap: it asserts the real running container's own scope identifies
family=cells, platform=rust, which pdf/net's and slides/python's containers can never produce.
"""
...
SERVICE_NAME = "serving-cells-rust"
...
def _initialize_body(request_id: int = 0) -> dict:
    return {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "tc-036-e2e-client", "version": "0.0.1"},
        },
    }
...
def test_report_index_freshness_identifies_the_real_cells_rust_deployment(
    session: _McpSession,
) -> None:
    ...
    This deployment must identify as family=cells, platform=rust; pdf/net's own container
    (family=pdf, platform=net) and slides/python's own container (family=slides,
    platform=python) can never produce this response. Without an assertion like this one,
    pointing BASE_URL at the wrong port is structurally incapable of failing here: every tool
    call against an EMPTY manifest store returns a byte-identical isError: false result no
    matter which real container answers it - only this field distinguishes them.
    """
    ...
    assert scope["family"] == "cells"
    assert scope["platform"] == "rust"
